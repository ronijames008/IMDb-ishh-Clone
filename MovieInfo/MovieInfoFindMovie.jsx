  ...
  const fetchMovieDetails = async (title) => {
    try {
      const response = await axios.get(
        `https://api.themoviedb.org/3/search/movie?api_key=bff2b86ea037d99d01532d860b4d4924&query=${encodeURIComponent(
          title,
  ...
  const getRecommendations = async () => {
    ...
    try {
      ...
      const genAI = new GoogleGenerativeAI(
        "AIzaSyDItXvLTy8khSJ4dmNXNhMelxdjYhTD5L4",
      );
        ...
        {watchlist.length === 0 ? (
          ...
          <>
            ...
            {recommendations.length > 0 && (
              <>
                <div className="grid grid-cols-2 justify-items-center gap-6 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
                  {recommendations.map((movie, index) => (
                    <div
                      key={index}
                      className="cursor-pointer overflow-hidden rounded-lg bg-gray-800 shadow-lg transition-transform duration-300 hover:scale-105 hover:transform"
